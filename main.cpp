	    ...
	    {	
			...
			switch(menu2) { // Nouveau switch pour le menu de gestion du magasin avec tout les choix à la clé
				...
				case 3: // Vu que l'user a rentré 3 on clear l'affichage et on arrive dans le menu de gestion des commandes du magasin
				{
					...
					std::cout << " 3°) Changer l'etat d'une commande" << std::endl;
					std::cout << " 4°) Afficher toutes les commandes" << std::endl;
					std::cout << " 5°) Afficher une commande en particulier" << std::endl;
					std::cout << "\n 0°) Retour" << std::endl;
					std::cout << "\n   --> Choisissez votre menu en tapant le numéro correspondant" << std::endl;
					std::cin >> menu3;
					...
					switch(menu3) { // Cette boucle permet de proposer à l'user de sélectionner les différentes possibilités du menu Commande
						...
						case 3:// Vu que l'user a rentré 2 on lui demande le numéro de commande mais aussi l'état de la commande
						{
							system("clear");
							std::cout << "------------------------- ETAT DE COMMANDE -------------------------" << std::endl;
							int num, ch;
							std::cout << "		Numéro de la commande = 0 --> Quitter" << std::endl;
							std::cout << "\nNuméro de la commande : ";
							std::cin >> num;
							if(num != 0){
								EasyStore.displayCommandeNum(num);
								std::cout << "\nChanger l'etat de la commande n°" << num << " :" << std::endl;
								std::cout << "	1°) En cours de préparation" << std::endl;
								std::cout << "	2°) Expédiée" << std::endl;
								std::cout << "	3°) Livrée" << std::endl;
								std::cout << "	4°) Annulée" << std::endl;
								std::cout << "\n	0°) Quitter" << std::endl;
								std::cin >> ch;
								if( ch != 0 ) {
									EasyStore.statusCommande(num, ch);
									std::cout << "\nEtat de la commande n°" << num << " modifié !" << std::endl;
									std::this_thread::sleep_for(std::chrono::milliseconds(2000));
								}
							}			
						}
						...
						case 4: // Vu que l'user a rentré 3 on clear l'affichage et on affiche toutes les commandes ainsi que leurs statuts
						{
							q = "0";
							system("clear");
							std::cout << "------------------------- COMMANDES DU MAGASIN -------------------------" << std::endl;
						...
						case 5: // Vu que l'user a rentré 4 on clear l'affichage et on va lancer un autre do while
						{
							int choix;
							do {
							q = "0";
